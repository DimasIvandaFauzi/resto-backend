...
export const allMenu = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Binds untuk filter WHERE (tanpa offset & limit)
        const filterBinds = {
            name: `%${name}%`,
            category: `%${category}%`,
            price,
        ...
        // Binds untuk query dengan pagination (termasuk offset & limit)
        ...
        const countResult = await conn.execute(countSql, filterBinds, { outFormat: OUT_FORMAT_OBJECT });
        const data = await conn.execute(sql, paginationBinds, { outFormat: OUT_FORMAT_OBJECT });
